/*
  ...
  2. Security
    ...
    - Add policy for authenticated users to manage all faculty entries
    - Add policy for public users to read active faculty entries
...
DO $$
BEGIN
  DROP POLICY IF EXISTS "Authenticated users can manage faculty members" ON faculty_members;
  DROP POLICY IF EXISTS "Active faculty members are publicly readable" ON faculty_members;
EXCEPTION
  WHEN OTHERS THEN
...
EXCEPTION
  WHEN OTHERS THEN
    -- Do nothing if trigger doesn't exist
END $$;

-- Insert sample data only if table is empty
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM faculty_members LIMIT 1) THEN
